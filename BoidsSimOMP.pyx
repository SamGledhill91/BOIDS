...
cdef int[:, :] GenerateNeighbours(double[:] x_values, double[:] y_values, int num_boids):
    cdef int neighbour
...
# cdef int[:,:] GenerateNeighbours(x_values, y_values, num_boids):
#     cdef int visual_range = 50
#     cdef int counter
#     cdef int[:,:] neighbour_list
...
def simulate_boids(int num_boids=1000, int num_frames=100, int width=1000, int height=1000,
    ...
    for frame_num in range(num_frames):
        ...
        for boid_outer in prange(num_boids, nogil=True, num_threads=4):
