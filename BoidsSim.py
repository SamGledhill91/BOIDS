...
def create_boids(num_boids, width, height):
    x_values = [random.uniform(0, width) for _ in range(num_boids)]
    y_values = [random.uniform(0, height) for _ in range(num_boids)]
    vx_values = [random.uniform(-5, 5) for _ in range(num_boids)]
    vy_values = [random.uniform(-5, 5) for _ in range(num_boids)]
...
def distance(boid1, boid2, x_values, y_values):
    return math.sqrt((x_values[boid1] - x_values[boid2])**2 + (y_values[boid1] - y_values[boid2])**2)
...
def update_velocity_and_position(boid_index, x_values, y_values, vx_values, vy_values, visual_range, min_distance, centering_factor, matching_factor, avoid_factor, speed_limit, width, height):
    ...
    # Limit speed
    speed = math.sqrt(vx_values[boid_index]**2 + vy_values[boid_index]**2)
...
# Example usage:
simulate_boids(num_boids=1000, num_frames=100, render=True)
